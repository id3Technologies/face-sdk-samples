    ...
    public void onLargestFaceProcessed(FaceProcessor.AnalyzeLargestFaceResult analyzeLargestFaceResult) {
        runOnUiThread(() -> {
            ...
            if (errorCode == 0) {
                ...
                FacePose facePose = analyzeLargestFaceResult.getFacePose();
                if (facePose.yaw < Parameters.yawMaxThreshold
                    ...
                    FaceOcclusionScores occlusionScores = analyzeLargestFaceResult.getFaceOcclusionScores();
                    if (occlusionScores.leftEyeOcclusionScore < Parameters.eyeOcclusionMaxThreshold
                            && occlusionScores.rightEyeOcclusionScore < Parameters.eyeOcclusionMaxThreshold
                            && occlusionScores.noseOcclusionScore < Parameters.noseOcclusionMaxThreshold
                            && occlusionScores.mouthOcclusionScore < Parameters.mouthOcclusionMaxThreshold) {
                        okLabel.setSpan(
                                new ForegroundColorSpan(colorOk),
                                0,
                                2,
                        ...
                        FaceAttributes faceAttributes = analyzeLargestFaceResult.getFaceAttributes();
