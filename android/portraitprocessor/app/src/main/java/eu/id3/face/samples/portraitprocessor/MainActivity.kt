    ...
    override fun onLargestFaceProcessed(analyzeLargestFaceResult: FaceProcessor.AnalyzeLargestFaceResult) {
        runOnUiThread {
            ...
            if (errorCode == 0) {
                ...
                val portraitAttributes = analyzeLargestFaceResult.getPortraitAttributes()

                /** Print age */
                analysisAnalysisText.append("Age : ")
                analysisAnalysisText.append(portraitAttributes.age.toString()).append("\n")


                /** Print expression */
                analysisAnalysisText.append("Expression : ")
                analysisAnalysisText.append(portraitAttributes.expression.name).append("\n")


                /** Print gender */
                val gender =
                    (if (portraitAttributes.genderMaleScore > 50) "Male" else "Female")
                analysisAnalysisText.append("Gender : ").append(gender).append("\n")


                /** Print global quality score */
                analysisAnalysisText.append("Quality : ")
                analysisAnalysisText.append(portraitAttributes.qualityScore.toString()).append("\n")

                analysisAnalysisText.append("\n")

                val facePose: FacePose = portraitAttributes.pose
                if (facePose.yaw.absoluteValue < Parameters.yawMaxThreshold && facePose.pitch.absoluteValue < Parameters.pitchMaxThreshold && facePose.roll.absoluteValue < Parameters.rollMaxThreshold) {
                    ...
                    if (portraitAttributes.leftEyeVisibilityScore >= Parameters.eyeOcclusionMaxThreshold
                        && portraitAttributes.rightEyeVisibilityScore >= Parameters.eyeOcclusionMaxThreshold
                        && portraitAttributes.noseVisibilityScore >= Parameters.noseOcclusionMaxThreshold
                        && portraitAttributes.mouthVisibilityScore >= Parameters.mouthOcclusionMaxThreshold
                        && portraitAttributes.glassesVisibilityScore < Parameters.glassesOcclusionMaxThreshold) {
                        okLabel.setSpan(
                            ForegroundColorSpan(colorOk),
                            0,
                            2,
                        ...
                        if (portraitAttributes.hatScore < Parameters.hatMaxThreshold) {
                            okLabel.setSpan(
                                ForegroundColorSpan(colorOk),
                                0,
                                2,
                        ...
                        if (portraitAttributes.mouthOpenScore < Parameters.mouthOpenMaxThreshold) {
                            okLabel.setSpan(
                                ForegroundColorSpan(colorOk),
                                0,
                                2,
                        ...
                        if (portraitAttributes.smileScore < Parameters.smileMaxThreshold) {
                            okLabel.setSpan(
                                ForegroundColorSpan(colorOk),
                                0,
                                2,
