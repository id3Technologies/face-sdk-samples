...
import eu.id3.face.samples.portraitprocessorjava.PortraitAttributes
...
class FaceProcessor(context: Context) {
    ...
    private var portraitCreated: Boolean = false
    ...
    init {
        try {
            ...
            FaceLibrary.loadModelBuffer(
                context.assets.open("models/face_age_estimator_v1a.id3nn").readBytes(),
                FaceModel.FACE_AGE_ESTIMATOR_1A, ProcessingUnit.CPU
            )
            FaceLibrary.loadModelBuffer(
                context.assets.open("models/face_expression_classifier_v1a.id3nn").readBytes(),
    ...
    fun analyzeLargestFace(
        ...
        val LOG_TAG = "PortraitProcessor"

        val portraitAttributes = PortraitAttributes()
        ...
        try {
            ...
            portraitAttributes.age = portrait.getAge()

            /** Get expression estimation. */
            processor.estimateExpression(portrait)
            portraitAttributes.expression = portrait.expression

            /** Get ICAO geometric attributes. */
            processor.estimateGeometryQuality(portrait)
            val geomAttributes = portrait.geometricAttributes

            /** Get face landmarks. */
            val landmarks = portrait.landmarks

            /** Get face attributes. */
            processor.detectOcclusions(portrait)
            processor.estimateFaceAttributes(portrait)

            val eyeGaze = portrait.eyeGaze

            /** Get ICAO portrait attributes */
            portraitAttributes.pose = portrait.pose
            portraitAttributes.leftEyeVisibilityScore = portrait.leftEyeVisibility
            portraitAttributes.rightEyeVisibilityScore = portrait.rightEyeVisibility
            portraitAttributes.mouthVisibilityScore = portrait.mouthVisibility
            portraitAttributes.noseVisibilityScore = portrait.noseVisibility
            portraitAttributes.glassesVisibilityScore = portrait.glasses
            portraitAttributes.genderMaleScore = portrait.genderMale
            portraitAttributes.hatScore = portrait.hat
            portraitAttributes.smileScore = portrait.smile
            portraitAttributes.mouthOpenScore = portrait.mouthOpening
            ...
            val icaoCheckpoints = portrait.qualityCheckpoints

            /*
             * get global quality score
             * NOTE: must be called after all estimations
             */
            portraitAttributes.qualityScore = portrait.qualityScore
            ...
            Log.v(LOG_TAG, "Error while processing portrait: " + e.message)
            return AnalyzeLargestFaceResult(ByteArray(0), PortraitAttributes(), e.errorCode)
        }
    }
    ...
    class AnalyzeLargestFaceResult(
        ...
        private var portraitAttributes: PortraitAttributes,
        ...
        fun getPortraitAttributes(): PortraitAttributes {
